    ...
    <el-container>
      ...
      <el-main>
        <el-table :data="filterTableData" style="width: 100%">
          ...
          <el-table-column label="科技属性">
            <template #default="{ row }">
              <!-- {{ row.Rating_List }} -->
              <p v-for="(item, index) in row.Rating_List">
                <span>{{ item.key }}</span>:
    ...
    <el-drawer v-model="drawer2" :direction="direction">
      <template #default>
        <el-form :model="form" style="max-width: 600px">
          ...
          <!-- 科技属性 -->
          <el-form-item label="科技属性:">
            <p style="display: flex;">
              <span>力量：</span>
              <el-input placeholder="(0-5)" style="width: 100px;" v-model="form.Science_Rating_Strength" />
            </p>
            <p style="display: flex;">
              <span>速度：</span>
              <el-input placeholder="(0-5)" style="width: 100px;" v-model="form.Science_Rating_Speed" />
            </p>
            <p style="display: flex;">
              <span>智能：</span>
              <el-input placeholder="(0-5)" style="width: 100px;" v-model="form.Science_Rating_Intelligence" />
            </p>
            <p style="display: flex;">
              <span>耐久：</span>
              <el-input placeholder="(0-5)" style="width: 100px;" v-model="form.Science_Rating_Durability" />
            </p>            
          </el-form-item>

          <el-form-item label="科技图片:">
            <el-upload class="avatar-uploader" action="http://127.0.0.1:3000/api/upload_Scienceimg" :show-file-list="false"
...
const beforeAvatarUpload: UploadProps['beforeUpload'] = (rawFile) => {
  ...
  form.Science_Image = `../../static/science_img/${form.Science_Name}.jpg`
  // 返回新的文件对象，以便上传时使用
  return newFile;
};
...
const filterTableData = computed(() =>
  tableData.value.filter(
    (data) =>
      !search.value ||
      data.Science_Name.toLowerCase().includes(search.value.toLowerCase())
  )
)
...
const handleDelete = async (index: number, row) => {
  ...
  let res = await delScience(row)
  if (res.success) {
    ElMessage({
      message: res.message,
      type: 'success',
    })
    await refreshScienceList()
  }
  // console.log(res);

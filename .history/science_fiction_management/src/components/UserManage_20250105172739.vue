  ...
  <el-drawer v-model="drawer2" :direction="direction" title="">
    <el-form :model="newUserForm" :rules="rules" ref="userFormRef" label-width="100px" class="demo-ruleForm">
      <el-form-item label="用户名" prop="username">
        <el-input v-show="!isupdate" v-model="newUserForm.username"></el-input>
        <el-input v-show="isupdate" v-model="newUserForm.username" disabled></el-input>
      </el-form-item>
      ...
      try {
        ...
        if(isupdate.value){
          res=await addUser({
            ...
            new_nickname:newUserForm.nickname,
            new_email:newUserForm.email
          },"update");
        }else{
          res=await addUser(newUserForm,"add");
        }
        console.log(res);

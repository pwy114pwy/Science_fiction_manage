    ...
    <el-container>
      <el-header>
        ...
        <el-select v-model="value2" filterable placeholder="请选择用户" style="width: 240px">
          <el-option label="全部" value="All" />
          <el-option v-for="item in user_list" :key="item.User_ID" :label="item.User_Name" :value="item.User_Nam" />
        </el-select>
      </el-header>
      <el-main>
...
const show_comment_list = computed(() => {
  ...
  if (value2.value !== 'All') {
    filteredComments = filteredComments.filter(comment => comment.User_Name == value2.value);
  }

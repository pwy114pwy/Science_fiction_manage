      ...
      <template #default>
        <el-form :model="form" style="max-width: 600px">
          ...
          <el-form-item label="书籍图片">
            <el-upload class="avatar-uploader" action="http://127.0.0.1:3000/api/upload_bookimg"
...
import { getBooks,addBooks } from '@/api/getBooks';
...
const onSubmit = async() => {
  ...
  let res=addBooks(JSON.stringify(form))
  try {
    const response = await fetch("http://127.0.0.1:3000/api/AddBook", {
      method: 'POST',
      headers: {

<template>
  <div class="content">
    <div class="hotbook">
    <div class="common-layout">
      <el-container>
        <el-header>
          <h2>热门图书</h2>
        </el-header>
        <el-main>
          <div id="main1" style="width: 400px;height:290px;"></div>
        </el-main>
      </el-container>
    </div>
  </div>
  <div class="browser">
    <div class="common-layout">
      <el-container>
        <el-header>
          <h2>点击量排名</h2>
        </el-header>
        <el-main>
          <div id="main2" style="width: 400px;height:290px;"></div>
        </el-main>
      </el-container>
    </div>
  </div>
  </div>



</template>

<script setup lang="ts">
import * as echarts from 'echarts';
import { onMounted, ref } from 'vue';
import { getEChartsOption1 } from '@/echartsConfig';
import { getEChartsOption2 } from '@/echartsConfig';
import { getAllCollectBook } from '@/api/getAllCollectBook';
import { Get_AllBrowse } from '@/base_api';
let chartDom1 = ref<HTMLElement | null>(null);
let chartDom2 = ref<HTMLElement | null>(null);
let myChart1: echarts.ECharts | null = null;
let myChart2: echarts.ECharts | null = null;
let collect_data = ref([])
let browser_data_x = ref([])
let browser_data_y = ref([])
function getcollectdata(collections) {
  // 使用 reduce 统计每本书籍的出现次数
  const bookCounts = collections.reduce((acc, curr) => {
    if (!acc[curr.Book_Name]) {
      acc[curr.Book_Name] = 0;
    }
    acc[curr.Book_Name] += 1;
    return acc;
  }, {});

  // 将统计结果转换为 [{Book_Name:"", value:"}] 格式的数组
  const result = Object.keys(bookCounts).map(key => ({
    name: key,
    value: bookCounts[key]
  }));
  return result
}
function getbrowserdata(browsers){
  console.log(browsers);
  for (const element of browsers) {
    if(!browser_data_x.value.includes(element.Book_Name)){
      browser_data_x.value.push(element.Book_Name)
    }
  }
  for (const element of browser_data_x.value) {
    for (const element1 of browsers) {
      if(element1.Book_Name==element){
        
      }
    }
  }
  console.log(browser_data_x.value);
  
  
}
onMounted(async () => {
  let res1 = await getAllCollectBook()
  let res2 = await getAllCollectBook()
  collect_data.value=getcollectdata(res1)
  getbrowserdata(res2)
  


  chartDom1.value = document.getElementById('main1');
  chartDom2.value = document.getElementById('main2');
  if (chartDom1.value) {
    myChart1 = echarts.init(chartDom1.value);
    const option = getEChartsOption1(collect_data.value)
    if (myChart1) {
      myChart1.setOption(option);
    }
  }
  if (chartDom2.value) {
    myChart2 = echarts.init(chartDom2.value);
    const option = getEChartsOption2(collect_data.value)
    if (myChart2) {
      myChart2.setOption(option);
    }
  }
});
</script>

<style lang="scss" scoped>
.el-header,
.el-footer {
  display: flex;
  justify-content: space-evenly;
  // background-color: #b3c0d1;
  height: 20px;
  color: var(--el-text-color-primary);
  text-align: center;
  line-height: 20px;

  span {
    font-size: 25px;
  }
}
.content{
  display: flex;
  justify-content: space-around;
}

.hotbook {
  width: 450px;
  height: 300px;
  position: relative;
  background-color: aqua;

  h1 {
    font-size: 20px;
  }
}
.browser {
  width: 450px;
  height: 300px;
  position: relative;
  background-color: aqua;

  h1 {
    font-size: 20px;
  }
}
</style>
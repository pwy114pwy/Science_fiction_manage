<template>
  ...
  <!-- 抽屉 -->
  <el-drawer v-model="drawer2" :direction="direction" title="添加用户">
...
import { getUsers,creaUser } from "@/api/getUsers"
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      try {
        await createUser(newUserForm, localStorage.getItem('token'));

    ...
    <el-main>
      <el-table :data="filterTableData" style="width: 100%">
        ...
        <el-table-column #default="{ row }" label="角色能力">
          <span v-show="row.AbilityList[0].key" v-for="(item,index) in row.AbilityList">{{item.key}}：{{ item.value }}</span>...
          <span v- v-for="(item,index) in row.AbilityList">{{item.key}}：{{ item.value }}</span>...
        </el-table-column>
    ...
    <template #default>
      <el-form :model="form" style="max-width: 600px">
        ...
        <el-form-item label="角色能力：">
          <div v-for="(item,index) in form.AbilityList">
            <!-- <el-input  v-model="item" placeholder=""  clearable /> -->
             {{ item }}
          </div>

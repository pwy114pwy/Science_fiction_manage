        ...
        <el-row :gutter="20">
          ...
          <el-col :span="3">
            <el-button @click="drawer2 = true" type="primary">
              添加作者<el-icon class="el-icon--right">
                <Upload />
              </el-icon>
            </el-button>
          </el-col>
      ...
      <template #default>
        <el-form :model="form" style="max-width: 600px">
          ...
          <el-form-item label="作者简介:">
            <el-input v-model="form.Author_Introduction" type="textarea" />
          </el-form-item>
          <el-form-item>
...
import { addAuthor } from '@/api/addAuthor';
import { delAuthor } from '@/api/deleteAuthor';
...
const handleEdit = (index: number, row) => {
  ...
  form.Author_ID = row.Author_ID; // 假设你的图书对象有一个id属性标识它是否是一个已有记录
  // 打开抽屉
  drawer2.value = true;
};
const handleDelete = async (index: number, row) => {
  // console.log(index, row)
  let res = await delAuthor(row)
  if (res.success) {
    ElMessage({
      message: res.message,
      type: 'success',

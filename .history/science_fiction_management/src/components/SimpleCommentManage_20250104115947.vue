    ...
    <el-container>
      <el-header>
        ...
        <el-select v-model="value2" filterable placeholder="请选择用户" style="width: 240px">
          <el-option label="全部" value="All" />
          <el-option v-for="item in user_list" :key="item.User_ID" :label="item.User_Name" :value="item.User_ID" />
        </el-select>
      </el-header>
      <el-main>
...
let show_comment_list = ref([])
let show2_comment_list = ref([])
let comment_list = ref([])
let user_list = ref([])
watch(value1, (newval, oldval) => {
  console.log(newval);
  if (newval == "All") {
    show_comment_list.value = comment_list.value
  } else {
    show_comment_list.value = comment_list.value.filter((item) => {
...
onMounted(async () => {
  ...
  let res3 = await getUsers(localStorage.getItem('token'))
  console.log(res3);
  book_options.value = res1
  comment_list.value = res2
  show_comment_list.value = res2

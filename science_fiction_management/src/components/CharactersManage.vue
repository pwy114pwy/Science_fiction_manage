<template>
    ...
    <el-main>
      <el-table :data="filterTableData" style="width: 100%">
        ...
        <el-table-column #default="{ row }" label="角色能力">
          <p v-show="row.AbilityList[0].value" v-for="(item, index) in row.AbilityList">
            <span>{{ item.key }}：</span>
            <span>{{ item.value}}</span>
          </p>
          <span v-show="!row.AbilityList[0].value">...</span>
    ...
    <template #default>
      ...
      <el-form :model="form" style="max-width: 600px">
        ...
        <el-form-item label="角色简介:">
          <el-input v-model="form.Role_Introduction" style="width: 240px" :rows="2" type="textarea"
             />
        </el-form-item>
        <!-- 经典语录 -->
        <el-form-item label="经典语录:">
          <el-input v-model="form.Classic_Quotes" style="width: 240px" :rows="2" type="textarea"
            placeholder="使用'|'间隔每一条语录" />
        </el-form-item>
        <!-- 标签 -->
        ...
        <!-- 经历 -->
        <el-form-item label="">
          <el-button type="primary" size="default" @click="add_event_btn">+</el-button>
        </el-form-item>
        ...
        <!-- 添加时能力 -->
        <!-- <el-form-item v-show="!form.AbilityList.length"  label="角色能力：">
  ...
  <el-dialog v-model="dialogVisible" title="" width="400">
    <el-input v-model="dialogContent" style="width: 100%" :rows="10" type="textarea" placeholder="输入模版：90(重要程度0-100)|三体入侵(标题)|三体人即将入侵地球...(正文)" />
    <template #footer>
  ...
  <!-- 添加弹窗 -->
  <el-dialog v-model="dialogVisible1" title="" width="400">
    <el-input v-model="dialogContent1" style="width: 100%" :rows="10" type="textarea" placeholder="输入模版：90(重要程度0-100)|三体入侵(标题)|三体人即将入侵地球...(正文)" />
    <template #footer>
      <div class="dialog-footer">
        <el-button @click="dialogVisible1 = false">Cancel</el-button>
        <el-button type="primary" @click="confirm_contribute_add">
          Confirm
        </el-button>
      </div>
    </template>
...
//确定添加经历按钮
function confirm_contribute_add() {
  dialogVisible1.value = false

  form.ContributeList.push(dialogContent1.value)
  dialogContent1.value = ""
}
...
const beforeAvatarUpload: UploadProps['beforeUpload'] = (rawFile) => {
  ...
  form.Role_Image = `../../static/role_image/${form.Role_Name}.jpg`
  // 返回新的文件对象，以便上传时使用
  return newFile;
};
...
const filterTableData = computed(() =>
  tableData.value.filter(
    (data) =>
      !search.value ||
      data.Illustrated_Name.toLowerCase().includes(search.value.toLowerCase())
  )
)
...
const handleDelete = async (index: number, row) => {
  ...
  let res = await delCharacter(row)
  if (res.success) {
    ElMessage({
      message: res.message,
      type: 'success',
...
function getdata(response) {
  ...
  for (const element of tableData.value) {
    ...
    for (const key in element) {
      if (key.includes('Contribute') && element[key]) {
